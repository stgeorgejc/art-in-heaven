...
class AIH_Art_Piece {
    ...
    /**
     * Get individual art piece statistics
     */
    public function get_piece_stats($id) {
        global $wpdb;
        $bids_table = AIH_Database::get_table('bids');
        ...
        $stats->total_bids = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $bids_table WHERE art_piece_id = %d", $id
        ));
        
        $stats->unique_bidders = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(DISTINCT bidder_id) FROM $bids_table WHERE art_piece_id = %d", $id
        ));
        
        $stats->highest_bid = $wpdb->get_var($wpdb->prepare(
            "SELECT MAX(bid_amount) FROM $bids_table WHERE art_piece_id = %d", $id
        ));
        
        $stats->lowest_bid = $wpdb->get_var($wpdb->prepare(
            "SELECT MIN(bid_amount) FROM $bids_table WHERE art_piece_id = %d", $id
        ));
        
        $stats->average_bid = $wpdb->get_var($wpdb->prepare(
            "SELECT AVG(bid_amount) FROM $bids_table WHERE art_piece_id = %d", $id
        ));
        
        // All bids for this piece
        $stats->all_bids = $wpdb->get_results($wpdb->prepare(
            "SELECT b.*, bd.name_first, bd.name_last, bd.email_primary, bd.confirmation_code
             FROM $bids_table b
        ...
        // Winning bidder info
        $stats->winning_bid = $wpdb->get_row($wpdb->prepare(
            "SELECT b.*, bd.name_first, bd.name_last, bd.email_primary, bd.phone_mobile, bd.confirmation_code
             FROM $bids_table b
             LEFT JOIN $bidders_table bd ON b.bidder_id = bd.confirmation_code
        ...
        $stats->is_sold = !empty($order_item);
        $stats->order_info = $order_item;
        $stats->is_paid = $order_item && $order_item->payment_status === 'paid';
    ...
    public function get_artists() {
        global $wpdb;
        return $wpdb->get_col("SELECT DISTINCT artist FROM {$this->table} WHERE artist != '' ORDER BY artist ASC");
    }
    
    /**
     * Get list of unique mediums
     */
    public function get_mediums() {
        global $wpdb;
        return $wpdb->get_col("SELECT DISTINCT medium FROM {$this->table} WHERE medium != '' ORDER BY medium ASC");
    }
    ...
    public function get_tiers() {
        global $wpdb;
        return $wpdb->get_col("SELECT DISTINCT tier FROM {$this->table} WHERE tier != '' ORDER BY tier ASC");
    }
