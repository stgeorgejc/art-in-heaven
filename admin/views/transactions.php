...
// Get pending orders for manual matching
$pending_orders = $wpdb->get_results(
    "SELECT id, order_number, total, created_at FROM {$orders_table} WHERE payment_status = 'pending' ORDER BY created_at DESC LIMIT 100"
);
?>
        ...
        <div class="aih-modal-body">
            ...
            <select id="aih-match-order-select" style="width: 100%; max-width: 400px;">
                ...
                <option value="<?php echo $order->id; ?>" data-amount="<?php echo $order->total; ?>">
                    <?php echo esc_html($order->order_number); ?> - $<?php echo number_format($order->total, 2); ?> (<?php echo date_i18n('M j', strtotime($order->created_at)); ?>)
                </option>
                <?php endforeach; ?>
            </select>
